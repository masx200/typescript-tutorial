...
(function (f, o, q, e, r) {
    ...
    var g = (MathJax.Menu = h.Subclass(
        {
            ...
            Post: function (M, E, B) {
                ...
                var C = window.getComputedStyle
                    ...
                    : { marginLeft: "0px" };
                var A =
                    u.right - Math.min(0, u.left) + parseFloat(C.marginLeft);
                if (!H && !F && "clientX" in M) {
                    H =
                        M.clientX +
                ...
                for (var w = 0, u = this.items.length; w < u; w++) {
                    if (this.items[w].name[y] === v) {
                        if (x.length) {
                            ...
                            return this.items[w].submenu.FindN(
                                y,
                                x[0],
                                x.slice(1)
                            );
    ...
    g.About = function (y) {
        ...
        g.About.GetJax(u, MathJax.OutputJax, [
            "OutputJax",
            "%1 Output Jax v%2",
        ]);
        g.About.GetJax(u, MathJax.ElementJax, [
            "ElementJax",
            "%1 Element Jax v%2",
        ]);
        u.push([
        ...
        if (g.prototype.msieAboutBug) {
            ...
            x.style.left =
                Math.floor(
                    (document.documentElement.scrollWidth - x.offsetWidth) / 2
                ) + "px";
            x.style.top =
                Math.floor((w - x.offsetHeight) / 3) +
                document.body.scrollTop +
                "px";
        } else {
            ...
            if (this.format === "Error") {
                ...
                if (s.semanticsAnnotations[this.format]) {
                    ...
                    if (g.jax.originalText == null) {
                        alert(
                            t("NoOriginalForm", "No original form available")
                        );
                        return;
                    }
    ...
    g.ShowSource.Text = function (z, x) {
        ...
        if (g.isMobile) {
            ...
            u.document.write(
                "<table><tr><td><pre>" + z + "</pre></td></tr></table>"
            );
            u.document.write("</body></html>");
            u.document.close();
        ...
        g.menu = g(
            ...
            c.SUBMENU(
                ...
                c.SUBMENU(
                    ...
                    c.RULE(),
                    c.RADIO(["TeXLocal", "TeX (local)"], "font", {
                        action: g.Font,
                    }),
                    ...
                    c.RULE(),
                    c.RADIO(["STIXLocal", "STIX (local)"], "font", {
                        action: g.Font,
                    }),
